...
我们将使用 Google 股票价格数据绘制时间序列线形图。 我们将数据（日期和每日收盘价，用逗号分隔）保存在没有表头的`.csv`文件中，因此我们将使用 Pandas 库进行读取并将其传递给`matplotlib.pyplot`函数以绘制图形。
...
标准API经常发生变化，有时候数据库网站没有响应。你需要在你的机器上安装这些API，因为他们没有随标准Python的发行版一起提供。
...
*   `plt.subplots()`允许我们根据图形的数量以及图形组织形式来定义`figure`的布局。 我们将在第 3 章“绘制多个图表，子图和图形”和第 6 章“带有高级功能的绘图”中了解更多信息。 在这种情况下，我们使用它来访问要绘制条形图的axes轴域，以便可以用代表条形图的实际数据对其进行标注。 如果您从第 1 章“Matplotlib 剖析”中回想起，我们已经看到轴域是`figure`中的单个图。
*   将月份格式从数字格式更改为 *x* 轴上的相应月份名称。`calendar.month_name[1:13]`将返回 1 月至 12 月，而`plt.xticks()`将 *x* 轴代号从数字 1 到 12，更改为 1 月到 12 月，以提高可读性。
*   `for`循环中的`ax.text()`用其相应的数据值标注每个条。 此函数的参数指定数据文本必须在条形图上精确放置：首先，获取当前条形图的 *x* 和 *y* 坐标，然后再添加`bar_width / 2`到 *x* 坐标，以及`1.002`高度到 *y* 纵坐标；然后，使用`va`和`ha`自变量，将文本居中对齐在条形上方：
...
前面的图也可以使用`plt.plot()`方法绘制。`plt.scatter()`方法具有更大的灵活性，可以用不同的大小、颜色等自定义每个点，我们将在气泡图一节中看到这一点。然而，这种灵活性是以性能为代价的。对于更大的数据集，`plt.plot()`方法比`plt.scatter()`方法快得多。

### 更多

在这里，我们将看到另一个散点图示例，在这里我们可以清楚地看到不同的段。

鸢尾花数据集（`Iris`）是最古老额数据集，由Ronald Fisher在1936年提出。该数据集有三个鸢尾画亚种的50个示例，三个亚种分别命名为`Setosa`、`Virginica`、和`Versicolor`。每个示例都有四个属性，以及萼片和花瓣的长度和宽度（以厘米为单位）。此数据广泛用于**机器学习**(**ML**) 的分类和聚类。我们将使用这个数据集来演示散点图如何在一个数据集中显示不同的聚类。
...
显然，我们可以在此处看到三个不同的群集。 但是，尚不清楚哪种颜色代表`setosa`，`versicolor`和`virginica`。 在随后的章节中，我们将看到如何使用标签来区分不同的聚类，在此我们将学习如何自定义图。

## 气泡图

使用相同的`plt.scatter()`方法绘制气泡图。 它是散点图的一种体现，图中的每个点都显示为气泡。 这些气泡中的每一个都可以显示为不同的颜色，大小和外观。
...
*   `Apr = [5, 7, 6, 8, 7, 9]`是按缺陷代码分类的4月份产品缺陷计数的列表。
*   `May = [0, 4, 3, 7, 8, 9]`是按缺陷代码分类的5月份产品缺陷计数的列表。
*   `June = [6, 7, 4, 5, 6, 8]`是按缺陷代码分类的6月份产品缺陷计数的列表。
...
*   `ax.legend(loc=2)`指定要在图的左上方绘制的图例； 我们将在第 4 章“开发可视化来提高发布质量”中了解有关图例位置的更多信息。
...
既然是一个原型的蜘蛛网，我们需要把最后一个点和第一个点连接起来，这样就有了一个循环流动的图。为了实现这一点，我们需要再次重复列表末尾的第一个部门数据点。因此在下面示例中，30和32（每个列表中的第一个条目）在结尾处再次重复。
...
*   虽然坐标需要以弧度给出，但等效度数很容易可视化。要将弧度转换为度数，可以使用`np.degrees` (`theta`)。
...
直方图用于绘制连续变量的分布。 连续变量值分为所需的段数，并绘制在 *x* 轴上，落在每个段中的值计数绘制在 *y* 轴上。 在 *y* 轴上，我们也可以绘制总数的百分比来代替计数，在这种情况下，它表示概率分布。 该图通常用于统计分析。
...
*   通过指定`nbins = 21`将此数据分为 21 个大小相等的段。
*   `plt.hist()`以`grp_exp`和`nbins`作为参数绘制直方图。 它返回三个参数`n`，`bins`和`patches`。`n`是包含每个段中有个项目的列表；`bins`是另一个指定箱的起点的列表，`patches`是每个箱中的对象的列表。 这些可以在程序的后面用于任何其他目的。
...
在 *y* 轴上，您可以绘制每个段中`grp_exp`列表中的所有条目的总和的百分比，而不是绘制频率， 通过在`plt.hist()`中指定`density=1`。 您还可以使用，数据的均值和标准差绘制近似正态分布，以查看此分布遵循正态分布的程度：
...
箱形图用于可视化连续变量的描述统计量。 它直观地显示了第一和第三四分位数，中值（平均值）和**四分位数间距**（**IQR**） 1.5 倍的拂线 -- 即第三四分位和第一四分位数之间的差值，超过此差值的是离群值。 第一四分位数（矩形框的底部）标记了一个点，总点数的 25% 位于该点之下。第三四分位数（矩形框的顶部）标记了一个点，表示 75% 的点数低于该点。
...
我们将在此示例中使用葡萄酒质量数据集的示例。 我们将考虑三个属性：`alcohol`，`fixed acidity`和`quality`。 通常，针对不同的维度（例如地理或时间）进行绘制，以比较数据在这些维度上的分布情况。 在这里，我们绘制了三个不同属性的箱形图：
...
*   `plt.boxplot()`是绘制箱形图的方法。`data`参数可以是一个或多个属性的列表。 默认情况下，每个框中的黄线代表属性的中值（也可以更改为平均值）。 下拂线位于方框底线的 1.5 IQR，上拂线位于方框上线的 1.5 IQR。 方框底线位于数据的第一四分位数，方框上线位于数据的第三四分位数。
...
在`plt.boxplot()`中，通过指定`showfliers=False`自变量 ，您可以抑制离群值，因此仅在两侧的拂线上绘图。 通过指定`vert=False`自变量 ，您可以水平绘制箱形图，还可以自定义具有不同形状和颜色的离群值：
...
*   `plt.violinplot()`绘制提琴图。 底部拂线是最小值，顶部拂线是最大值，水平线在平均值。 垂直线两侧是代表数据实际分布的数据直方图。 通过将`showmeans=True`替换为`showmedian=True`，我们可以用一条横线代表数据的中位数，而不是均值：
...
热图用于以不同的强度可视化不同颜色的数据范围。 在这里，我们以绘制相关矩阵作为热图为例。 相关矩阵的元素指示两个变量之间的线性关系的强度，并且矩阵包含给定数据中属性的所有组合的相关值。 如果数据具有五个属性，则相关矩阵将是`5 x 5`矩阵。
...
等高线图通常用于显示误差如何随机器学习算法（例如线性回归）中优化系数的变化而变化。 如果线性回归系数为`theta0`和`theta1`，并且预测值和实际值之间的误差为`Loss`，则对于给定的`Loss`值，`theta0`和`theta1`的所有值 ]形成轮廓。 对于`Loss`的不同值，通过更改和`theta1`的值形成不同的轮廓。
