...
Matplotlib 具有三层架构：**backend**，**artist**和**scripting**，它们在逻辑上组织为栈。 `scripting`是开发人员用来创建图形的API。 `artist`完成了内部创建图形的实际工作。 `backend`是显示图形的位置。
...
这是栈的中间层。 Matplotlib 使用`artist`对象绘制图形的各种元素。 因此，我们在图形中看到的每个元素（请参见图中的元素）都是一个`artist`。 该层提供了**面向对象的API** ，以最大的灵活性来绘制图形。 该接口适用于经验丰富的 Python 程序员，他们可以创建复杂的仪表板应用。
...
包含输出图所有元素的高级 Matplotlib 对象称为`figure`。 可以以不同方式排列多个图形以形成`figure`。 `figure`的每个元素都是可自定义的。

#### Figure图表

下图是`figure`的剖析图，其中包含其所有元素：

![](img/adf3f22d-db1c-4cc5-b8c7-43d39ce56e6a.png)

Anatomy of a figure (Source : http://diagramss.us/plotting-a-graph-in-matlab.html)

#### Axes轴域

`axes`是`figure`的子部分，其中绘制了图形。`axes`具有**标题**，**x 标签**和 **y 标签**。`figure`可以具有许多这样的`axes`，每个`axes`代表一个或多个图形。 在上图中，只有一个`axes`，两条蓝色和红色的折线图。

#### Axis轴

这些是数字线，代表要绘制的`figure`的比例。 二维图具有 *x* 轴和 *y* 轴，而三维图具有 *x* 轴， *y* 轴 ，以及 *z* 轴。

不要混淆`axes`和`axis`。`axis`是`axes`的一个元素。从语法上讲，`axes`也是`axis`的复数形式，因此需要根据上下文解释`axes`的含义，是否引用了多个`axis`元素还是指向了一个`axes`对象。

#### Label标签

这是`figure`中各个元素的名称，例如 *x* 轴标签， *y* 轴标签，图形标签（上个图中的蓝色信号/红色信号，*图形剖析图*）等等。

#### Legend图例

当`axes`中有多个图形时（如上图，*图形剖析图*），每个图形都有其自己的标签，所有这些标签表示为`Legend`。 在上图中，图例位于图的右上角。

#### Title标题

它是每个`axes`的名称。 当`figure`具有多个带有各自标题的`axes`时，`figure`也可以具有自己的标题。 上图只有一个轴域，因此`axes`和`figure`都只有一个标题。

#### Ticklabels刻度标签

每个`axis`（*x*，*y* 或 *z*）都有一个范围的值，该值分为许多相等的段。 段分为两个级别。 在上一个图表中（剖析图）， *x* 轴刻度范围从 0 到 4，分为四个主要段（0-1、1-2、2-3 和 3-4），每个主要段又分为四个次要段（0-0.25、0.25-0.5 和 0.5-0.75）。 主段两侧的刻度称为**主刻度**，次段称为**次刻度**，它们的名称为**主刻度标签**和**次刻度标签**。
...
如果您使用的是 Jupyter 笔记本，则只有在执行给定单元格中的所有语句后才能看到输出，因此您必须将这三个语句中的每一个放在单独的单元格中，然后一个接一个地执行，以查看图形在每个代码语句之后被更新。

在老版本的 Matplotlib 或一些后端（如 `macosx`）， 图形可能不会被立即更新。在这种情况下，应在最后显式的调用`plt.draw()`以显示图形。
...
在交互模式下，我们看到了随着每条指令逐步构建的图形。 在非交互模式下，您将提供构建图形的所有指令，然后使用命令显式的显示图形。
...
![](img/d402e05e-7f6a-4ede-8d2a-af1d38568344.png)

最新版本的Jupyter Notebook似乎在没有明确调用`plt.show()`命令的情况下显示该图。但是，在Python shell或嵌入式应用中，需要调用`plt.show()`或`plt.draw()`才能在屏幕上显示图表。
...
`.csv`文件具有行和列的关系数据库结构，`test.csv`文件在 2D 空间中具有五个点的 *x*，*y* 坐标。 每个点是文件中的一行，分为两列：`x`和`y`。 使用相同的 NumPy `loadtxt()`函数用于加载数据：
...
根据数据的格式和结构，我们将不得不使用 Python，NumPy 或 pandas 函数来读取数据并将其重新格式化为可以传入`matplotlib.pyplot`函数的适当结构。 之后，按照通常的绘制说明绘制所需的图形。
...
应当注意，这些参数将在本会话中的所有绘制图形所使用。如果你想在同一个会话中使用不同的参数配置，应使用`polt`命令中提供的属性。
