...
正如我们从第一章中学到的，轴域是绘制图形的空间，通常在图中看到的所有元素都是轴域的一部分。 我们在一幅图表中可以有多个轴域。 在本秘籍中，我们将学习如何在同一轴域上绘制多个图表。
...
到目前为止，我们已经学习了如何在单个轴域进行多图绘制和一个图表中的多个轴域上绘制多个图。 在本秘籍中，我们将学习如何在给定的会话中绘制多个图形。
...
`basic_units.py` 并不像其他所有 Python 标准软件包。它是一个用户自定义的 Python 程序，需要复制到工作目录中。然后，在主程序中，从这个程序中导入函数，就像我们导入其他包或用户定义的程序一样。
